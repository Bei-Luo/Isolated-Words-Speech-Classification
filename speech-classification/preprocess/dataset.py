...
class DataFeed(object):
    ...
    def get_path(self, stu, cate, ith):
        ...
        ret = os.path.join(self.data_dir, stuid, "{0}-{1:02}-{2:02}.wav".format(stuid, cate, ith + 1))
        return ret
...
def spec_cvloader(folds, nfold, batch_size, num_workers=4, shuffle=True, cuda=True):
...
def spec_loader(candidates, batch_size, num_workers=4, shuffle=True, cuda=True):
    d = DataFeed()
    paths = []
    for c in candidates:
        for i in range(class_num):
